...
const InfluencerCollab: React.FC = () => {
  ...
  return (
    <section className="py-24 bg-gradient-to-b from-gray-50 to-white relative overflow-hidden">
      {/* Decorative elements */}
      <div className="absolute top-0 left-0 w-64 h-64 bg-indigo-100 rounded-full -translate-x-1/2 -translate-y-1/2 opacity-70"></div>
      <div className="absolute bottom-0 right-0 w-96 h-96 bg-indigo-50 rounded-full translate-x-1/3 translate-y-1/3"></div>
      ...
      <div className="max-w-7xl mx-auto px-6 relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          {influencers.map(influencer => (
            ...
            >
              <div className="aspect-square relative overflow-hidden bg-indigo-100">
              ...
              <div className="p-6">
                <div className="flex justify-between items-start mb-3">
                  <div>
                    ...
                    <p className="text-indigo-600 text-sm font-medium">{influencer.role}</p>
                  </div>
                  <div className="bg-indigo-50 rounded-full px-3 py-1 flex items-center gap-1">
                    <Star size={14} className="text-indigo-600" />
                    <span className="text-indigo-600 font-medium text-sm">{influencer.followers}</span>
                  </div>
                </div>
                
                <div className="flex items-center gap-3 mt-4">
                  {influencer.instagram && (
                    <a href={`https://instagram.com/${influencer.instagram}`} className="text-gray-500 hover:text-indigo-600 transition-colors">
                      <Instagram size={20} />
                    </a>
                  )}
                  {influencer.twitter && (
                    <a href={`https://twitter.com/${influencer.twitter}`} className="text-gray-500 hover:text-indigo-600 transition-colors">
                      <Twitter size={20} />
                    </a>
                  )}
        ...
        <div className="mt-16 text-center">
          <a href="#" className="inline-flex items-center gap-2 bg-indigo-600 hover:bg-indigo-700 text-white px-8 py-3 rounded-lg font-medium transition-colors duration-300">
